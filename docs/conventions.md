...
- **github-automation's housekeeping.yaml intentionally deviates from the standard caller template**: This repo's `housekeeping.yaml` includes two jobs not present in the standard template (and not in `template-gitops` or `template-generic`): `mechanic-dispatch` (dispatches `engineering-agent` on every new issue) and `bulk-assign` (a `workflow_dispatch` utility for bulk-assigning open issues). These are platform-repo-specific additions — consuming repos should use the 8-job standard template from `docs/adoption.md`.
