...
- **Platform-specific caller (github-automation's own housekeeping.yaml)**: github-automation's `.github/workflows/housekeeping.yaml` includes two jobs not present in the standard caller template — `mechanic-dispatch` (dispatches `engineering-agent` on new issues, bypassing bot-event suppression) and `bulk-assign` (`workflow_dispatch` utility for bulk issue assignment). These are intentional additions specific to this platform repo. Consuming repos should use the standard template in [docs/adoption.md](adoption.md) and omit these jobs.
