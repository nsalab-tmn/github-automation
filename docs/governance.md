...
|---|---|---|
| Org-wide rulesets | Branch protection across all repos | Requires GitHub Team plan |

### Repo-level (free tier)
...
### Repo-level (public repos only on free tier)

| Setting | What it enforces |
|---|---|
| Require PR before merge | No direct pushes to main |
| Require status check: "PR Validation" | PRs must pass validation |
| Block force push | History cannot be rewritten |

### Free tier limitations

Branch protection and org-wide rulesets require a paid plan for private repos. Public repos get branch protection on the free tier. See [issue #51 comment](https://github.com/nsalab-tmn/github-automation/issues/51) for details.

### GitHub Projects automation

GitHub Projects V2 has its own automation rules that operate independently of GitHub Actions:

- Auto-add issues from specific repos
- Set default field values (Status, Priority, Size) when items are added
- Move items between columns based on state changes

These fire regardless of who created the issue (including bots), unlike GitHub Actions workflows.
...
projects:
  - name: cheburnet
    knowledge-base: nsalab-tmn/cheburnet-knowledge-base
    conventions-path: conventions
    repos:
      - nsalab-tmn/cheburnet-knowledge-base
      - nsalab-tmn/cheburnet-vpn
      - ...
```
...
| Project board operations | GitHub App token | `nsalab-automation[bot]` |
...
All secrets live in this repo only (no duplication across project repos). The GitHub App (`nsalab-automation`) is installed org-wide.
